...
# Automated Tools
Using [ManSpider](https://github.com/blacklanternsecurity/MANSPIDER)
```bash
# Scan accross a network, searching for interesting file extensions
manspider.py 192.168.0.0/24 -e xlsx xls ods doc docx csv json xml bat com vbs ps1 psd1 psm1 pem key rsa pub reg pfx cfg conf config vmdk vhd vdi dit txt kdbx kdb 1pif agilekeychain opvault lpd dashlane psafe3 pws enpass bwdb msecure stickypass pwm rdb safe zps pmvault mywallet jpass pwmdb pfx p12 pkcs12 pem key crt cer csr jks keystore key keys der ppk ovpn sh py env ini rdp yml yaml cred go sql dump sqlite sqlite3 db db3 fdb gdb mdb accdb bak mdf ldf dmp -d domain.local -u user -p password

# Searching for interesting file names
manspider.py -f id_rsa id_dsa id_ecdsa id_ed25519 authorized_keys known_hosts hosts .bash_history .zsh_history .mysql_history .psql_history ConsoleHost_history.txt SAM SYSTEM SECURITY secret sitemanager.xml NTDS.dit passwd shadow passwords.txt credentials.txt mdp.txt ips.txt passw user admin account network login logon cred unattend.xml wp-config.php config.php settings.py -d domain.local -u user -p password

# Searching files containing interesting words
manspider.py  -d domain.local -u user -p password
```
...
Vérifier vulns sur l'absence de signature SMB
