...
Without **administrative access** to WordPress, it may be possible to `upload .php` files as a `plugin`. Create a `php reverse shell` in a file called `shell.php`. Then we can add a new plugin in the `Plugin` menu 